...
RELEASE_TAG=${PORTAL_RELEASE_TAG:-v0.19.0}
OUTPUT_DIR=${1:-$PWD}
...
export PORTAL_DOCKER_CONFIG_DIR=$docker_config
docker --config "$docker_config" pull "$image"
docker --config "$docker_config" pull "$COSIGN_IMAGE"
bash "$SCRIPT_DIR/verify-published-image.sh" "$image" "$RELEASE_TAG" "$REPOSITORY" private
