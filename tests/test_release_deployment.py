...
IMAGE = f"ghcr.io/{REPOSITORY.lower()}@sha256:{DIGEST}"
OFFLINE_IMAGE = f"ghcr.io/{REPOSITORY.lower()}:offline-0.25.0"
...
def test_compose_wrapper_uses_only_the_base_file_in_source_mode(tmp_path):
    ...
    env_file.write_text(
        "PORTAL_DEPLOY_MODE=source\nPORTAL_IMAGE=proxmox-vm-portal:0.25.0\n",
        encoding="utf-8",
    )
...
def test_cosign_verification_is_exact_and_confined(tmp_path):
    ...
    assert (
        ...
        ".github/workflows/release.yml@refs/tags/v0.25.0"
    ) in arguments
